...
class TestUserModel:
    ...
    async def test_get_user_with_followers_and_following_by_username(self, users_and_followers, get_session_test,
                                                                     lookup_by):
        async with get_session_test as session:
            if lookup_by == "username":
                ...
                response = await User.get_user_with_followers_and_following(session, user_id=users_and_followers[0].id)
    ...
    async def test_get_user_with_followers_and_following_missing_args(self, get_session_test):
        async with get_session_test as session:
            ...
            self.assert_error_response(response, "Missing one of argument (username or user_id)")
